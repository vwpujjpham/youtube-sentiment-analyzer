...
import pandas as pd
from sentiment import styled_sentiment_label, overall_sentiment, weighted_percentages
...
percpositive,percneutral,percnegative = weighted_percentages(likes,sentiments)
df = pd.DataFrame({
    "Comment": comments,
    "Like Count": likes,
...
with analysis_tab:
    with st.container():
        if video_link and "www.youtube.com/watch?v=" in video_link:
            st.subheader(f"Weighted Sentiment Score: {weighted_score}")
            st.subheader(f"Overall Sentiment: {overall}")
            st.write(f"Comments Processed: {len(likes)}")
            st.write(f"Postive: {math.ceil(percpositive * 100)}")
            st.write(f"Neutral: {math.ceil(percneutral * 100)}")
            st.write(f"Negative: {math.ceil(percnegative * 100)}")
